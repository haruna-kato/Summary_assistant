...
App.controller("TimelineCtrl", function ($scope) {
  ...
  $scope.project = {
    ...
    layers: [
      ...
      {id: "Text", number: 998, y: 0, label: "Text Track", lock: false},
      {id: "Silence", number: 999, y: 0, label: "Cut Track", lock: false}
    ],
    markers: [],
    progress: {}
  };
  
  // console.log("Layers: " + JSON.stringify($scope.project.layers));
  ...
  $scope.movePlayhead = function (position_seconds) {
    ...
    var trackOffset = 140; // トラック名幅
    // トラック幅を考慮
    $(".playhead-line").css("left", ($scope.project.playhead_position * $scope.pixelsPerSecond + trackOffset) + "px");
...
$scope.updateRecentItemJSON = function (item_type, item_ids, tid) {
  ...
  console.log(item_type,item_ids)
...
$scope.updateLayerIndex = function () {
  ...
  $scope.loadJson = function (EntireProjectJson) {
    // console.log("Full project JSON:", JSON.stringify(EntireProjectJson.value, null, 2));

    $scope.$apply(function () {
      // Update the entire JSON object for the entire timeline
      // console.log("EntireProjectJson.value.layers length:", EntireProjectJson.value.layers.length);
      // console.log(EntireProjectJson.value.layers);
      // console.log("length",$scope.project.layers.length)
      //コメントアウト
      $scope.project = EntireProjectJson.value;
      // console.log("length0",$scope.project.layers.length)
    ...
    //  Silence layer を作る
    var hasSilence = $scope.project.layers.some(layer => layer.id === "Silence");
    if (!hasSilence) {
      $scope.project.layers.push({
        id: "Silence",
        number: 999,
        y: 0,
        label: "Cut Track",
    ...
    //  Text layer を作る
    var hasText = $scope.project.layers.some(layer => layer.id === "Text");
    if (!hasText) {
      $scope.project.layers.push({
        id: "Text",
        number: 998,
  ...
  //追加
  $scope.getSortedLayers = function () {
    return $scope.project.layers.slice().sort(function (a, b) {
      if (a.id === "Silence") return -1; // Silenceは常に上
      if (b.id === "Silence") return 1;

      // Text は Silence の次に来る
      if (a.id === "Text") {
        // a が Text の場合、b が Silence 以外なら上に
        return -1;
      }
      if (b.id === "Text") {
        // b が Text の場合、a が Silence 以外なら下に
        return 1;
      }

      return b.number - a.number; // 通常通り逆順
    });
  };
  $scope.getSortedClips = function () {
    return $scope.project.clips.slice().sort(function (a, b) {
      // Silenceレイヤーは最上位（番号が高いが上にしたいので逆順）
      const aLayer = (a.layer === 999) ? Infinity :
                   (a.layer === 998 ? 9999 : a.layer); // Text は Silence の次に
      const bLayer = (b.layer === 999) ? Infinity :
                   (b.layer === 998 ? 9999 : b.layer);
  
      // layerが大きいほど上に（表示上は先に描画）
  ...
  function computeHighlightsFromSilences(ranges, clipStart, clipEnd) {
    // clipStart/clipEnd: クリップの開始・終了時間
    let highlights = [];
    let prevEnd = clipStart;
   
    ranges.forEach(s => {
        let start = Math.max(s.from, clipStart);
        let end = Math.min(s.to, clipEnd);
        // 無音区間が正しい範囲なら追加
         if (end > start) {
          highlights.push({from: start, to: end});
        }
    });
    return highlights;
  }

  $scope.silencesPerClip = {};
  $scope.summaryPerClip = {};
  $scope.sentClipPaths = {}; // 送信済み判定フラグ
  $scope.checkedTextClips = {}; // Textトラック警告用
  $scope.alertedInvalidVideo = false; // ← 一度だけ警告するためのフラグ

  $scope.getSortedClipEffects = function (clip) {
    if (!clip || !clip.effects) return [];

    const id = clip.id;

    // ---------- 動画トラック処理 ----------
    if (clip.layer === 999 && !$scope.sentClipPaths[id]) {
      const path = clip.reader.path;
      const video_extensions = [".mp4", ".mov", ".avi", ".mkv", ".webm", ".flv", ".wav"];
      const isVideo = video_extensions.some(ext => path.endsWith(ext));

      if (isVideo) {
        $scope.sentClipPaths[id] = true;
        console.log('送信 path (動画):', path);

        res_time = timeline.detect_silent(path);

        timeline.silenceDetected.connect(function(jsonStr) {
          const silences = JSON.parse(jsonStr);
          $scope.project.clips.forEach(clip => {
            const clipPath = clip.reader.path;
            if (clip.layer === 999 && video_extensions.some(ext => clipPath.endsWith(ext))) {
              clip.highlights = computeHighlightsFromSilences(silences, clip.start, clip.end);
              updateTimelineHighlights(clip.id, clip.highlights, silences);
              $scope.silencesPerClip[clip.id] = silences;
            }
          });
        });
      }
      else{
          if (!$scope.alertedInvalidVideo) {
            timeline.show_message("warn","Cutトラックに対象外のクリップが存在します。");
            $scope.alertedInvalidVideo = true;
          }
          console.log("対象外クリップ:", clip.id, path);
      }
    }

    // ---------- Textトラック処理 ----------
    else if (clip.layer === 998) {
      const path = clip.reader.path;
      const text_extensions = [".txt", ".srt", ".csv", ".log", ".md",".json"];
      const isText = text_extensions.some(ext => path.endsWith(ext));

      // pathが無い、または拡張子が非テキスト → 警告
      if (!path || !isText) {
          if (!$scope.checkedTextClips[id]) {
              timeline.show_message("warn", "Textトラックに非テキストクリップが存在します: " + clip.id);
              $scope.checkedTextClips[id] = true; // 一度だけ警告
          }
          return []; // 非テキストは処理しない
      }
      // 有効なテキストの場合
      if (!$scope.checkedTextClips[id]) {
        console.log("テキストクリップ検出:", path);
        $scope.checkedTextClips[id] = true;
      }
    }

    // ---------- エフェクトソート ----------
    return clip.effects.slice().sort(function (a, b) {
      const aLayer = (a.layer === 999) ? Infinity :
                    (a.layer === 998 ? 9999 : a.layer);
      const bLayer = (b.layer === 999) ? Infinity :
                    (b.layer === 998 ? 9999 : b.layer);
      return bLayer - aLayer;
    });
  ...
  $scope.getSortedTransitions = function () {
    return $scope.project.effects.slice().sort(function (a, b) {
      const aLayer = (a.layer === 999) ? Infinity :
                   (a.layer === 998 ? 9999 : a.layer);
      const bLayer = (b.layer === 999) ? Infinity :
                   (b.layer === 998 ? 9999 : b.layer);
  ...
  function updateTimelineHighlights(clipId, highlights, ranges) {
      const clipDiv = document.getElementById("clip_" + clipId);
      if (!clipDiv) return;

      // 既存のハイライトをクリア
      const old = clipDiv.querySelectorAll(".highlight");
      old.forEach(h => h.remove());
      highlights.forEach((h,index) => {
        const div = document.createElement("div");
        div.className = "highlight";
        div.style.position = "absolute";
        div.style.left = (h.from * $scope.pixelsPerSecond) + "px";   // 修正
        div.style.width = ((h.to - h.from) * $scope.pixelsPerSecond) + "px";  // 修正
        div.style.top = "0px";
        ...
        div.dataset.silenceIndex = index;
        // ハイライトの移動イベント
        div.addEventListener("mousedown", function(e) {
          e.stopPropagation();   // クリックが下の動画に伝わるのを防ぐ
          e.preventDefault();    // ブラウザのデフォルト動作（選択やドラッグ）を防ぐ
          ...
          let startWidth = parseFloat(div.style.width);
      
          let isLeftHandle = offsetX < 10;           // 左端10px
          let isRightHandle = offsetX > rect.width - 10; // 右端10px
      
          function onMouseMove(e) {
              const delta = e.clientX - startX;
      
              if (isRightHandle) {
                  div.style.width = Math.max(1, startWidth + delta) + "px";
                  h.to = h.from + parseFloat(div.style.width) / $scope.pixelsPerSecond;
              } else if (isLeftHandle) {
                  div.style.left = startLeft + delta + "px";
                  div.style.width = Math.max(1, startWidth - delta) + "px";
                  h.from = parseFloat(div.style.left) / $scope.pixelsPerSecond;
              }

              // ranges 配列も更新
              const silenceIndex = parseInt(div.dataset.silenceIndex);
              if (ranges[silenceIndex]) {
                  ranges[silenceIndex].from = h.from;
                  ranges[silenceIndex].to = h.to;
              }
          }
        ...
        clipDiv.appendChild(div);
      });
      // console.log("finish")
      // console.log("clip length:", clip.end - clip.start);
      // console.log("timeline width:", clipDiv.offsetWidth);
      // console.log("calculated width:", (clip.end - clip.start) * $scope.pixelsPerSecond);
  }
  



  // $scope.cutSilences = function() {
  //   console.log("cut silent!");
  

  //   if (!Object.keys($scope.silencesPerClip).length) {
  //     timeline.show_message("warn","無音区間が検出されていません。\n解析を実行してから再度お試しください。");
  //     return;
  //   }

  //   let hasSilence = false;

  //   Object.keys($scope.silencesPerClip).forEach(clipId => {
  //     const silences = $scope.silencesPerClip[clipId];
  //     console.log("silences:", silences);

  //     if (!silences || !silences.length) {
  //       timeline.show_message("error",`クリップ ${clipId} に無音区間はありません。`);
  //       return;
  //     }

  //     hasSilence = true;

  //     const clip = $scope.project.clips.find(c => c.id === clipId);
  //     if (!clip) return;

  //     // 無音区間を削除するタイム範囲を作る
  //     const cutRanges = silences.map(s => ({ start: s.from, end: s.to }));
  //     console.log("このクリップでカットする範囲:", cutRanges);

  //     // Python 側に送信
  //     timeline.cut_silences(
  //       clip.reader.path,
  //       JSON.stringify(cutRanges) // 文字列化
  //     );
  //   });

  //   // 全クリップに無音がなかった場合
  //   if (!hasSilence) {
  //     timeline.show_message("info","全てのクリップに無音区間がありません。");
  //   }
  // };

  // ボタン用ラッパー
  $scope.cutSilences = function() {
    const selbox = document.getElementById("menu"); // select 要素を取得
    const mode = selbox.value;
    const useSummary = mode === "sum";      // 選択されている option の値を取得
    console.log(`cutMode: ${selbox.value}, useSummary: ${useSummary}`);
    $scope.cutRangesForClips(useSummary, mode);
  };

  $scope.cutRangesForClips = function(useSummary = false, mode = "cut") {
    const clipRanges = useSummary ? $scope.summaryPerClip : $scope.silencesPerClip;
    const rangeTypeName = useSummary ? "要約区間" : "無音区間";

    console.log(`${rangeTypeName}`);

    if (!Object.keys(clipRanges).length) {
        timeline.show_message("warn", `${rangeTypeName} が検出されていません。\n解析を実行してから再度お試しください。`);
        return;
    }

    let hasRange = false;

    Object.keys(clipRanges).forEach(clipId => {
        const ranges = clipRanges[clipId];
        console.log(`${rangeTypeName} for clip ${clipId}:`, ranges);

        if (!ranges || !ranges.length) {
            timeline.show_message("error", `クリップ ${clipId} に ${rangeTypeName} はありません。`);
            return;
        }
        ...
        const cutRanges = ranges.map(r => ({ start: r.from, end: r.to }));
        console.log(`このクリップで処理する範囲:`, cutRanges);
    ...
    if (!hasRange) {
        timeline.show_message("info", `全てのクリップに ${rangeTypeName} はありません。`);
    }
  };

//テキストとビデオ
// $scope.processVideoAndText = function() {
//   console.log("click!")
...
//   const text_ext = [".txt", ".srt", ".csv", ".log", ".md", ".json"];

//   // プロジェクト内のすべてのクリップをチェック
//   $scope.project.clips.forEach(clip => {
//     if (!clip.reader || !clip.reader.path) return;
//     const path = clip.reader.path;

//     if (clip.layer === 999 && video_ext.some(ext => path.endsWith(ext))) {
//       videoPath = path;
//       console.log("setVideo:", videoPath);
//     }

//     if (clip.layer === 998 && text_ext.some(ext => path.endsWith(ext))) {
//       textPath = path;
//       console.log("setText :", textPath);
//     }
//   });

//   if (videoPath && textPath) {
//     console.log("[JS] Found both video and text");
//     console.log("Video:", videoPath);
//     console.log("Text :", textPath);

//     timeline.process_video_and_text(videoPath, textPath);
//   } else {
//     // エラー
//     if (!videoPath) timeline.show_message("error", "動画トラック(999)が見つかりません。");
//     if (!textPath) timeline.show_message("error", "テキストトラック(998)が見つかりません");
//   }
// };

// 動画 + 手順 text を Python に送る関数
$scope.onOpenEditorClicked = function() {
  console.log("openVideoTextWindow")
  const video_ext = [".mp4", ".mov", ".avi", ".mkv", ".webm", ".flv"];
  const text_ext = [".txt", ".srt", ".csv", ".log", ".md", ".json"];
  ...
  if (videoPath && textPath) {
    ...
    timeline.open_video_text_window(videoPath, textPath);
    // ハイライト用
    // summaryDetected シグナルを受け取る
    timeline.summaryDetected.connect(function(jsonStr) {
      const summaryRanges = JSON.parse(jsonStr);
      console.log("[JS] Received summary ranges:", summaryRanges);
      // 対応する動画クリップに highligts を設定
      $scope.project.clips.forEach(clip => {
        const clipPath = clip.reader.path;
        if (clip.layer === 999 && video_ext.some(ext => clipPath.endsWith(ext))) {
          clip.highlights = computeHighlightsFromSilences(summaryRanges, clip.start, clip.end);
            updateTimelineHighlights(clip.id, clip.highlights, summaryRanges);
            $scope.summaryPerClip[clip.id] = summaryRanges;
          }
        });
      });
  } else {
