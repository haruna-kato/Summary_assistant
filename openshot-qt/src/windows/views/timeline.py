...
from PyQt5.QtCore import QProcess,  QVariant
from PyQt5.QtWidgets import QMessageBox

import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))  # src の一つ上の階層
...
class TimelineView(updates.UpdateInterface, ViewClass):
    ...
    #追加
    silenceDetected = pyqtSignal(str)
    summaryDetected = pyqtSignal(str)
    ...
    def get_clip_by_id(self, clip_id):
        for clip in self.window.project.clips:  # self.window.project.clips は Clip オブジェクトのリスト
            if clip.id == clip_id:
                return clip
        return None
    ...
    @pyqtSlot(str)
    def detect_silent(self, file_path):
        print(f"[Python] detect_silent called with: {file_path}")
        timeline_dir = os.path.dirname(os.path.abspath(__file__))  # src/windows/views
        cut_path = os.path.abspath(os.path.join(timeline_dir, "../../timeline/silence_api.py"))
        if not os.path.exists(cut_path):
            print(f"[ERROR] not found at {cut_path}")
            return

        # # JS に送信
        self.process = QProcess(self)  # QProcess をインスタンス変数に保持
        print(f"[Python] detect_silent : {cut_path}")
        # 終了時の処理
        self.process.finished.connect(self.handle_silence_finished)

        # 起動
        print(f"[Python] detect_silent run: {cut_path}")
        self.process.start(sys.executable, [cut_path, file_path])
        
    def handle_silence_finished(self):
        output = self.process.readAllStandardOutput().data().decode()
        print("[TimelineView] output:", output)
        self.silenceDetected.emit(output)

    @pyqtSlot(str, str, str)
    def cut_silences(self, path, cut_ranges_json, mode="cut"):
        print("[TimelineView] starting cut_worker", flush=True)

        worker_path = os.path.abspath(os.path.join(
            os.path.dirname(__file__), "../../timeline/cut_worker.py"
        ))
        print(f"[TimelineView] worker_path: {worker_path}", flush=True)

        self.worker = QProcess(self)
        self.worker.setProcessChannelMode(QProcess.MergedChannels)  # stdout+stderrをまとめる
        self.worker.readyReadStandardOutput.connect(self.handle_cut_output)
        self.worker.finished.connect(self.handle_cut_finished)

        self.worker.start(sys.executable, [worker_path, path, cut_ranges_json, mode])


    def handle_cut_output(self):
        output = bytes(self.worker.readAllStandardOutput()).decode()
        print("[Worker output]", output)


    def handle_cut_finished(self):
        print("[TimelineView] Cut worker finished", flush=True)
    
    @pyqtSlot(str, str)
    def process_video_and_text(self, videoPath, textPath):
        print("Video in Timeline:", videoPath)
        print("Text in Timeline:", textPath)

    @pyqtSlot(str, str)
    def show_message(self, level, message):
        _ = get_app()._tr  # 翻訳関数を取得
        if level == "info":
            QMessageBox.information(self, _("Information"), message)
        elif level == "warn":
            QMessageBox.warning(self, _("Warning"), message)
        elif level == "error":
            QMessageBox.critical(self, _("Error"), message)
        else:
            QMessageBox.information(self, _("Message"), message)
            
    @pyqtSlot(str, result=str)
    def trans_mes(self, mes):
        _ = get_app()._tr
        return _(mes)
    
    @pyqtSlot(str, str)  
    def open_video_text_window(self, video_path, text_Path):
        print("Received request to open VideoTextWindow")

        # 要約モデルへvideo_path, text_pathを渡す
        # videoの要約候補[from~,to~-]
        # 手順とタイムスタンプ   {"start": 0, "end": 5, "text": "手順1: ダミー開始"},
        video_json, steps_json = self.summary_model(video_path, text_Path)
        print("Using dummy steps:", steps_json)

        def create_window():
            from windows.video_text_window import VideoTextWindow
            
            file = File.get(path=video_path)
            if not hasattr(self, "_video_text_window") or self._video_text_window is None:
                # print("Init window in Timeline")
                self._video_text_window = VideoTextWindow(file, steps_json, parent=self)
                self._video_text_window.setObjectName("video_text_window")
                self._video_text_window.resize(1200, 600)
                self._video_text_window.show()
            else:
                # print("Update window in Timeline")
                # 既存ウィンドウがあれば内容更新
                # 既存ウィンドウがある場合は update_content_safe で Clip を更新
                self._video_text_window.update_content_safe(file, steps_json)
                self._video_text_window.show()
                self._video_text_window.raise_()
            QTimer.singleShot(0, lambda: self.summaryDetected.emit(video_json))

        # メインスレッドに投げる
        QTimer.singleShot(0, create_window)
    
    # 要約モデルの代わり
    def summary_model(self, video_path, text_path):
        print("rum summary")
        dummy_steps = [
            {"start": 0, "end": 5, "text": "手順1: ダミー開始"},
            {"start": 6, "end": 12, "text": "手順2: ダミー続行"},
            {"start": 13, "end": 20, "text": "手順3: ダミー終了"}
        ]
        steps_json = json.dumps(dummy_steps)
        dummy_sum = [{"from": 0.0, "to": 10.988}, 
                     {"from": 22.855, "to": 26.021}, 
                     {"from": 47.621, "to": 62.997}]
        video_json = json.dumps(dummy_sum)
        # self.summaryDetected.emit(video_json)

        return video_json, steps_json
