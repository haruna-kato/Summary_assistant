...
class ZoomSlider(QWidget, updates.UpdateInterface):
    ...
    def clear_all_clips(self):
        """libopenshotの全クリップ・トランジション・マーカーを削除"""
        for clip in Clip.filter():
            try:
                clip.delete()
        ...
        #         tr.delete()
        #     except Exception as e:
        #         print(f"Error deleting transition {tr.id}: {e}")

        # for marker in Marker.filter():
        #     try:
        #         marker.delete()
        #     except Exception as e:
        #         print(f"Error deleting marker {marker.id}: {e}")

        # # GUIキャッシュもリセット（ZoomSliderなど）
        # win = get_app().window
        # if hasattr(win, "sliderZoomWidget"):
        #     win.sliderZoomWidget.clip_rects.clear()
        #     win.sliderZoomWidget.clip_rects_selected.clear()
        #     win.sliderZoomWidget.marker_rects.clear()
        #     win.sliderZoomWidget.repaint()

        print("✅ All clips, transitions, and markers cleared.")
