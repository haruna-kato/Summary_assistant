...
class MainWindow(updates.UpdateWatcher, QMainWindow):
    ...
    def open_project(self, file_path, clear_thumbnails=True):
        ...
        try:
            if os.path.exists(file_path):
                ...
                print("zoom init in main in open")
    ...
    def actionPreview_File_trigger(self, checked=True):
        ...
        if not f:
            ...
            return
        # ファイルタイプ判定
        ext = os.path.splitext(f.absolute_path())[1].lower()
        if ext in [".txt", ".md", ".csv", ".json"]:  # テキストファイルの場合
            from windows.text_preview import TextPreview
            log.info("text file")
            self._preview_window = TextPreview(f, preview=True)
            self._preview_window.setObjectName("text_preview")
            self._preview_window.resize(800, 600)  # サイズを指定
    ...
    def __init__(self, *args):
        ...
        #追加
        self.actionPreview_Text = QAction("Preview Text", self)
        self.actionPreview_Text.setObjectName("actionPreview_Text")
